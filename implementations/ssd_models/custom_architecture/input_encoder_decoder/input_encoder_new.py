...
class SSDInputEncoder:
    ...
    def __call__(self, ground_truth_labels):
        ...
        non_background_rows = y_encoded[:, :, self.background_id] == 0
        
        grid_centers_x = y_encoded[:, :, -2]
        grid_centers_y = y_encoded[:, :, -1]
        
        x_coords = y_encoded[:, :, [-10, -8, -6, -4]]
        y_coords = y_encoded[:, :, [-9, -7, -5, -3]]
        
        y_encoded[:, :, [-10, -8, -6, -4]] = np.where(non_background_rows[:, :, np.newaxis], x_coords - grid_centers_x[:, :, np.newaxis], x_coords)
        y_encoded[:, :, [-9, -7, -5, -3]] = np.where(non_background_rows[:, :, np.newaxis], y_coords - grid_centers_y[:, :, np.newaxis], y_coords)

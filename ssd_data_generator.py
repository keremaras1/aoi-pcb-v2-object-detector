...
def split_labels(labels_parentdir, split_ratios=[0.7, 0.9]):
    assert max(split_ratios) <= 1.0, 'Incorrect ratios. Ratios should not exceed 1.0'
    all_labels = pd.read_csv(os.path.join(labels_parentdir, "labels_train.csv"))
    n_labels = all_labels.shape[0]

    df_train = all_labels[:int(n_labels * split_ratios[0])]
    df_trainval = all_labels[int(n_labels * split_ratios[0]):int(n_labels * split_ratios[1])]
    df_val = all_labels[int(n_labels * split_ratios[1]):]

    df_train.to_csv(os.path.join(labels_parentdir, 'labels_train.csv'), index=False)
    df_trainval.to_csv(os.path.join(labels_parentdir, 'labels_trainval.csv'), index=False)
    df_val.to_csv(os.path.join(labels_parentdir, 'labels_val.csv'), index=False)


class SSD_Data_Generator():
    def __init__(self, ic, background, rotation_range, ic_count, black=True):
        if black:
            size = get_ic_scale()
            self.ic = Image.new('RGBA', size, color='black')
        else:
            self.ic = Image.open(ic)
        self.background_ref = background
        self.background = Image.open(background).copy()
    ...
    def get_shift(self, rotated_ic):
        ...
        image_shift_x = random.randint(0, b_arr.shape[0] - ic_arr.shape[0] - 1)
        image_shift_y = random.randint(0, b_arr.shape[1] - ic_arr.shape[1] - 1)
    ...
    def get_final_image(self):
        ...
        self.background = Image.open(self.background_ref).copy()
        return final

    def generate_image(self, bb=False, show=False):
        ...
        if show:
            self.get_final_image().show()

        return self.get_final_image()

    def generate_dataset(self, parent_dir='/Users/keremaras/AOI_Project/Journal_Code/Datasets/self_generated',
    ...
    def gt_list_to_label(self, name, ic=0):
        for gt in self.gt_list:
    ...
    def save_labels(self, parent_dir, split):
        ...
        assert split == 'train' or split == 'val' or split == 'trainval', 'Wrong split type. Must be: train, val or trainval'
        ...
        if split == 'train':
            ...
            DF.to_csv(os.path.join(parent_dir, 'labels_trainval.csv'), header=header, index=False)


# data_generator = SSD_Data_Generator('U26_ic.png', 'green_background.png', rotation_range=30, ic_count=10)

# data_generator.generate_dataset(size=10000)

split_labels('/Users/keremaras/AOI_Project/Journal_Code/Datasets/self_generated')
